...
/**
 * Draws a light/dark checker pattern behind icon previews so that
 * transparent or near-background-colored shapes remain visible
 * instead of blending in and appearing as parsing failures.
 */
    ...
    public void draw(@NonNull Canvas canvas) {
        ...
        for (int row = 0; row < rows; row++) {
            for (int col = 0; col < cols; col++) {
                ...
                canvas.drawRect(
                        left, top,
                        left + CELL_SIZE_PX, top + CELL_SIZE_PX,
