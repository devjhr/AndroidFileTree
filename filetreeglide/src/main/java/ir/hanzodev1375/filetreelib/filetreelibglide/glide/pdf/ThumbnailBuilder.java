...
public class ThumbnailBuilder implements ModelLoader<String, Bitmap> {
  ...
  public LoadData<Bitmap> buildLoadData(
      @NonNull String input, int width, int height, @NonNull Options options) {
    return new LoadData<>(
        new ObjectKey(input), new ThumbnailCreator(mContext, input));
  }

  @Override
  public boolean handles(@NonNull String input) {
    return input.endsWith(".pdf".toLowerCase());
  }
  ...
  private static class ThumbnailCreator implements DataFetcher<Bitmap> {
    ...
    public void loadData(
        @NonNull Priority priority, @NonNull DataCallback<? super Bitmap> callback) {
      try {
