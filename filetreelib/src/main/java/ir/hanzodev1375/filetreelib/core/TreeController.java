...
public final class TreeController {
  ...
  /**
   * Expands {@code node}. If the node has not yet loaded its children, triggers a lazy load via the
  ...
  public void renameNode(
    ...
    if (newName.isEmpty()) {
      if (callback != null) {
        callback.onRenameFailed(node, new IllegalArgumentException("New name must not be empty"));
      }
      return;
    }
   ...
   */
  @MainThread
  public void deleteNode(@NonNull TreeNode node, @Nullable DeleteCallback callback) {
  ...
  public void createFolder(
      @NonNull TreeNode parent, @NonNull String name, @Nullable CreateCallback callback) {
  ...
  private void createEntry(
      @NonNull TreeNode parent, @NonNull String name, int type, @Nullable CreateCallback callback) {

    if (name.isEmpty()) {
      if (callback != null) {
        callback.onCreateFailed(name, new IllegalArgumentException("Name must not be empty"));
      }
      return;
    }
  ...
  private int findAlphabeticalInsertIndex(
      @NonNull TreeNode parent, @NonNull TreeNode newNode, int type) {
  ...
  private void applyExpandedState(@NonNull TreeNode node, @NonNull Set<String> expandedIds) {
  ...
  public interface PasteCallback {
    @MainThread
    void onPasted(@NonNull List<TreeNode> pastedNodes);

    @MainThread
    void onPasteFailed(@NonNull Exception error);
  }

  // ─────────────────────────────────────────────
  // selectAll() — add after clearSelection()
  // ─────────────────────────────────────────────

  /**
   * Selects all currently visible nodes. Updates SelectionManager's internal ordered list before
   * selecting.
   */
  @MainThread
  public void selectAll() {
    selectionManager.setVisibleNodes(visibleList.snapshot());
    selectionManager.selectAll();
  }

  // ─────────────────────────────────────────────
  // pasteNodes() — add after selectAll()
  // ─────────────────────────────────────────────

  /**
   * Pastes clipboard nodes into {@code destination} asynchronously. If {@code isCut} is true,
   * originals are removed from the model after a successful copy (move semantics).
   *
   * @param destination target folder node
   * @param nodes list from {@link ClipboardManager#getClipboard()}
   * @param isCut true = move, false = copy
   * @param callback result callback (may be null)
   */
  @MainThread
  public void pasteNodes(
      @NonNull TreeNode destination,
    ...
    backgroundExecutor.submit(
        () -> {
          try {
            // Delegate actual file-system work to the provider
            List<TreeNode> pasted = dataProvider.pasteNodes(destination, snapshot, isCut);

            mainHandler.post(
                () -> {
                  // Insert each new node into the model
                  for (TreeNode n : pasted) {
                    int idx = findAlphabeticalInsertIndex(destination, n, n.getType());
                    model.insertNodeAt(destination, idx, n);
                  }
                  // If cut, remove originals from model
                  if (isCut) {
                    for (TreeNode src : snapshot) {
                      expandManager.collapse(src);
                      model.removeNode(src);
                      cache.invalidate(src.getId());
                    }
                  }
                  // Expand destination so pasted nodes are visible
                  if (!destination.isExpanded()) expandManager.expand(destination);
                  cache.invalidate(destination.getId());
                  if (callback != null) callback.onPasted(pasted);
                });

          } catch (Exception e) {
            mainHandler.post(
                () -> {
                  if (callback != null) callback.onPasteFailed(e);
                });
          }
        });
  }
